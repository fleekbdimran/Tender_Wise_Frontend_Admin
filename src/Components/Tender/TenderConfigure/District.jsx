...
function AddCategoryModal({ isOpen, onClose }) {
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      const response = await ApiClient.post("/admin/tender-config/district", {
        name: categoryName,
        division_id: selectedCategory?.id || "",
      });
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      const response = await ApiClient.patch(
        `/admin/tender-config/district/${category.id}`,
        {
          name: categoryName,
          status: Number(categoryStatus),
          division_id: selectedCategory.id // Send the selected category's ID
        }
      );
...
function District() {
    ...
    const fetchCategories = async () => {
      ...
      try {
        const response = await ApiClient.get(
          `/admin/tender-config/district?${queryParams.toString()}`
        );
  ...
  return (
    <div className="h-screen w-full flex flex-col p-4 bg-gray-100 gap-2">
      <h2 className="text-2xl font-semibold mb-4 text-gray-800">District={totalItems}</h2>
