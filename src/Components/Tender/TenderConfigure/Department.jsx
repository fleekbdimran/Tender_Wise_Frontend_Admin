...
function AddCategoryModal({ isOpen, onClose }) {
  ...
  const handleFormSubmit = async (e) => {
    ...
    console.log(categoryName); // ডেটা দেখার জন্য
    try {
...
function EditCategoryModal({ isOpen, onClose, category, onSubmit }) {
  ...
  const [categoryStatus, setCategoryStatus] = useState(category ? category.status : "");
  ...
  const handleFormSubmit = async (e) => {
    e.preventDefault();
    console.log(categoryStatus); // ডেটা দেখার জন্য
    // API কল করা
    try {
        ...
        <form onSubmit={handleFormSubmit} className="space-y-6">
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => setCategoryStatus(e.target.value)} // Updated to dynamically set the current status
              className="mt-1 block w-full px-4 py-3 border border-gray-300 rounded-md text-sm shadow-sm focus:outline-none focus:ring-teal-500 focus:border-teal-500"
            >
...
function Department() {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      ...
      if (type) queryParams.append("status", type);
  ...
  useEffect(() => {
    const filtered = categories.filter((category) => {
      ...
      const matchesType = type ? category.department_status === parseInt(type) : true;  // `type` এর মান থাকলে তা দিয়ে ফিল্টার হবে
            ...
            {currentItems.map((category, index) => (
              <tr key={index} className="hover:bg-gray-50">
                ...
                <td className="px-4 py-2 border-b">
                  <span
                    className={`px-2 py-1 rounded-full text-xs cursor-pointer ${category.department_status === 1
                      ? "bg-green-100 text-green-700"
                      : "bg-red-100 text-red-700"
                      }`}
                  >
                    {category.department_status === 1 ? "Available" : "Unavailable"}
                  </span>
                </td>
                <td className="px-4 py-2 border-b">
                  <button
