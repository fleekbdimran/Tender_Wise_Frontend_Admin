...

{
  ...
  const handleSubmit = async e => {
    ...
    if (!name || !selectedCategory) {
     ...
     // Map status to 1 (available) or 0 (unavailable)
     const status = currentStatus === 'available' ? 1 : 0;

    const data = {
      name,
      category_id: selectedCategory,
      status: status,  // Set status as 1 for available or 0 for unavailable  
    };
    console.log(data);
    ...
    try {
      ...
      console.log(error.response?.data); // Log the response body to check for detailed error message
      Swal.fire({
        title: "Error!",
        text: error.response?.data?.message || "Something went wrong!",
        icon: "error",
