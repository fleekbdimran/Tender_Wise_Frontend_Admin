...
function AddCategoryModal({ isOpen, onClose }) {
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      const response = await ApiClient.post("/admin/tender-config/sector", {
        name: categoryName,
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      const response = await ApiClient.patch(
        `/admin/tender-config/sector/${category.id}`,
        {
          name: categoryName,
...
const [totalItems, setTotalItems] = useState(0);
  ...
  useEffect(() => {
    const filtered = categories.filter((category) => {
      ...
      const matchesType = type ? category.sector_status === parseInt(type) : true;  
