...
//     setModalTitle(title);
//     setCurrentStatus(District ? District.status : 'Available');
//     setIsModalOpen(true);
//   };
...
//   const filteredDistricts = District.filter(amenity => 
...
//                       District.status === 1 ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
//                     }`}
//                   >
...
function DesignationTypeModal({
  ...
  const handleSubmit = async e => {
    ...
    try {
      const token = localStorage.getItem('authToken'); // Make sure your token is saved in localStorage after login
      const response = await ApiClient.post('/admin/tender-config/District', data, {
        headers: {
